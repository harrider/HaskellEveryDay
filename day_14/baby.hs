...
-- ($) -- this function is called "function application".  It's type signature looks like the following:
-- ($) :: (a -> b) -> a -> b
-- f $ x = f x
--
-- ** IMPORTANT: Precedence: 
-- 	- A function with a space between the function name and parameters has the highest precedence 
-- 		in Haskell
--
-- 	- Function application using ($) has the lowest precedence in Haskell
--
-- 	Function application:
-- 		- a space -> left associative (so f a b c is the same as ((f a) b) c)
--
-- 		- ($) -> right associative. Most of the time it's a convenince function that helps
-- 			us not have to write so many parenthesis
--
-- 			- sum (map sqrt [1..130]) can be rewritten as sum $ map sqrt [1..130]
--
-- 			- sqrt (3 + 4 + 9) can be rewritten as sqrt $ 3 + 4 + 9
--
-- 			- sum (filter (> 10) (map (*2) [2..10])) can be rewritten as
-- 				sum $ filter (> 10) $ map (*2) [2..10]
--
-- 		- Apart from getting rid of parenthesis, $ means that function application
-- 			can be treated just like another funciton, so for instance, you can
-- 			map function application over a list of functions
--
-- 			- ghci> map ($ 3) [(4+), (10*), (^2), sqrt] 	// output = [7.0, 30.0, 9.0, 1.732]


-- (.) -- this function is used for "function composition".  Its type signature looks like the following:
-- (.) :: (b -> c) -> (a -> b) -> a -> c
-- f . g = \x -> f (g x)
--
--	- The function 'f' has to take as input, something of the same type as function 'g' returns
--
--	- One use for function composition is making functions on the fly to pass to other functions.
--		- Lambdas can be used for that, but many times, function composition is clearer
--			and more concise

-- -- A function to negate all numbers is a list and return that as the value using lambdas:
-- ghci> map (\x -> negate (abs x)) [5,-3,-6,7,-3,2,-19,24]	// output = [-5,-3,-6,-7,-3,-2,-19,-24]


-- -- The above function rewritten using function composition:
-- ghci> map (negate . abs) [5,-3,-6,7,-3,2,-19,24]	// output = [-5,-3,-6,-7,-3,-2,-19,-24]
...
-- -- With the above in mind, the following use of a lambda and multiple parenthesis would look like:
-- ghci> map (\xs -> negate (sum (tail xs))) [[1..5],[3..6],[1..7]] 	// output = [-14,-15,-27]

-- -- The above can be turned into the following using function composition:
-- ghci> map (negate . sum . tail) [[1..5],[3..6],[1..7]]		// output = [-14,-15,-27]


-- Composing functions that take several parameters usually requires paritally applying the functions
-- 	so that each function takes just one parameter

-- -- The following function:
-- ghci> sum (replicate 5 (max 6.7 8.9)

-- -- can be rewritten like using function composition:
-- ghci> (sum . replicate 5 . max 6.7) 8.9

-- -- or even more clearly using both function composition and function application:
-- ghci> sum . replicate 5 . max 6.7 $ 8.9



-- -- The following function uses multiple parenthesis to determine function composition:
-- ghci> replicate 100 (product (map (*3) (zipWith max [1,2,3,4,5] [4,5,6,7,8])))


-- -- It can be rewritten using function application and function composition:
-- ghci> replicate 100 . product . map (*3) . zipWith max [1,2,3,4,5] $ [4,5,6,7,8]


-- Another commuse use of function composition is defining function in so called "point-free" style
-- 	(also called pointless style)

-- Earlier we defined a sum function as:
-- sum' :: (Num a) => [a] -> a
-- sum' xs = foldl (+) 0 xs
--
-- The "xs" can be ommitted since it appears on both sides of the equation and the function could be
-- 	rewritten.  This function creates a function that takes a list as a paremeter.
--
-- 	- Rewriting in this style is called "point free style"
sumPF :: (Num a) => [a] -> a
sumPF = foldl (+) 0


-- -- An example function that can be rewritten in point free style:
-- fn x = ceiling (negate (tan (cos (max 50))))

-- -- The previous function rewritten in point free style:
-- fn = ceiling . negate . tan . cos . max 50


-- Earlier we talked about a function that would find the sum of all odd squares that are smaller than
-- 	10,000.  This is what that function looked like:
oddSquareSum :: Integer
oddSquareSum = sum (takeWhile (<10000) (filter odd (map (^2) [1..])))


-- Rewriting the above function using function composition would look like:
offSquareSum' :: Integer
oddSquareSum' = sum . takeWhile (<10000) . filter odd . map (^2) $ [1..]


-- Idiomatic Haskell calls for not going too crazy with function composition though and 'let' binding
-- 	is more preferred to break a function up into easier to read names / functions
offSquareSum'' :: Integer
offSquareSum'' = 
